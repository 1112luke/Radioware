...
import data_sink
import noncoherent_demodulation
...
def getsamples(q1, Mode, fs_in,total_samples, debugging, same_machine, ftuned, samples_perbit, device):

        #Initialize the receiver device
        ain, spi_desc, sdr, dc_offset_data_I, dc_offset_data_Q = HAL.setup_rx_device_control(device, same_machine, ftuned, fs_in, total_samples)
...
if __name__ == "__main__":
        ...
        p1 = multiprocessing.Process(target=getsamples, args=(q1, Mode,fs_in,total_samples, debugging, same_machine, None, samples_perbit,device)) # "getsamples" process
        ...
        while(Mode == 1 or Mode == 2 or Mode == 4):
                ...
                a_demodulated, baseband_symbols = noncoherent_demodulation.noncoherent_demodulation_wireline(data, samples_perbit, payload_start, N, fs_in, Ts_in, preamble_length, showplot)
                if isinstance(baseband_symbols, int):
                        print("frame sync error detected")
